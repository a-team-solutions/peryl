...
export type Manage = <S>(view: View<S>, onAction: OnAction<S>, state?: S) => HsmlFnc | Hsmls;
    ...
    return (e: Element) => {
        if ((e as any).ctrl) {
            ...
            if (w.view.name === view.name) {
                ...
                w.umount();
                const w1 = new Ctrl(state, view, onAction);
                if (state !== undefined) {
                    w1.state = state;
                }
                w1.mount(e);
            ...
            const w = new Ctrl(state, view, onAction);
            if (state !== undefined) {
                w.state = state;
            }
            w.mount(e);
...
export class Ctrl<S> implements HsmlHandlerCtx {
    ...
    constructor(state: S, view: View<S>, onAction: OnAction<S>) {
        this.state = state;
        this.view = view;
